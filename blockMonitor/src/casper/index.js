...
const BlockMonitor = { // BlockMonitor is a class which contains methods to monitor blocks
	...
	load: foundTx => { // load is a function which starts the block monitor
		BlockMonitor.onFoundTx = foundTx // set callback function
		BlockMonitor.lastBlockId = parseInt(fs.readFileSync( BlockMonitor.lastBlockFile )) // load last block height from file
	...
	setBlock: _=> fs.writeFileSync( BlockMonitor.lastBlockFile, BlockMonitor.lastBlockId.toString() ), // setBlock is a function which sets last block height to file
	checkLoadAccounts: _=> { // checkLoadAccounts is a function which checks if accounts need to be loaded
		const loadAccounts = ( !BlockMonitor.accounts ) // loadAccounts is a flag which indicates if accounts need to be loaded
		if( fs.existsSync( BlockMonitor.addressUpdateFile ) )	{ // check if address update file exists
			fs.unlinkSync( BlockMonitor.addressUpdateFile ) // delete address update file
			loadAccounts = true // set loadAccounts flag to true
		}
		if( loadAccounts )	{ // do accounts need to be loaded
			BlockMonitor.accounts = JSON.parse(fs.readFileSync( BlockMonitor.addressesFile )) // load accounts from file
		}
	},
	check: async _=> { // check is a function which checks block height and returns transactions in the block
		if( BlockMonitor.running )	{	// if block monitor is running
			return // return
		}
		...
		while( BlockMonitor.lastBlockId < thisBlockId )	{ // while last block height is less than this block height
			BlockMonitor.checkLoadAccounts() // check if accounts need to be loaded
			const transfers = await RPC.getBlockTransfers( ( await RPC.getBlockInfoByHeight( BlockMonitor.lastBlockId ) ).hash ) // get transfers in the block
			for( let value of transfers )	{ // for each transfer
				if( (BlockMonitor.accounts[value.from] || BlockMonitor.accounts[value.to]) && BlockMonitor.onFoundTx )	{ // if transfer is from or to an account
...
const callback = (blockId, transaction) => { // callback is a function which is called when transaction is found
