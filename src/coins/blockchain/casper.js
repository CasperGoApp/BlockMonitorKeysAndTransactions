...
const RPC = require("../../../blockMonitor/src/casper/rpc") //import the RPC object
...
const getStateHash = async _=> RPC.getStateRootHash( ( await RPC.getLatestBlock() ).hash ) // get the state hash

const getAccount = async ( accountSeed, index, includePrivateKey, isRetry ) => { // get the account
  let account = isRetry? isRetry: null // if the account is retry, set it to the account
  if( !account ) { // if the account is not retry, get the account from the seed
    const hdWallet = new CasperHDWallet( accountSeed? accountSeed: seed, EncryptionType.Ed25519 ) // create the hd wallet
  ...
  try	{
    publicAccount = await RPC.getAccountInfo( ( await RPC.getLatestBlock() ).hash, account.public.address ) // get the public account from the blockchain
    if( publicAccount )	{ // if the public account exists
      account.hashString = publicAccount.account_hash // set the hash string
      account.uref = publicAccount.main_purse // set the uref
      account.balance = ( await RPC.getAccountBalance( await getStateHash(), account.uref ) ) / 1000000000 // set the balance
    }	else if( !isRetry ) { // if the public account does not exist and the account is not retry
      await sendTransfer( new Date().getTime(), 'casper' + ( TESTNET? '-test': '' ), sourceAccount, account.public.address, NEW_ACCOUNT_AMOUNT ) // send the new account the min amount to index in the blockchain
  ...
  if( includePrivateKey && ( !account.privateKey || isRetry ) )	{ // if the private key is included and the private key is not set or the account is retry
    const hdWallet = new CasperHDWallet( accountSeed? accountSeed: seed, EncryptionType.Ed25519 ) // create the hd wallet
    const userAccount = await hdWallet.getAccount( index ) // get the account
...
const sendTransfer = async (id, networkName, fromAccount, to, amount) => { // send a transfer
  const casperClient = new CasperClient(RPC.url) // create the casper client
  const signKeyPair = Keys.Ed25519.parseKeyPair(Buffer.from( fromAccount.public.key, 'hex' ), Buffer.from( fromAccount.privateKey, 'hex' ) ) // create the sign key pair
  const deployParams = new DeployUtil.DeployParams( signKeyPair.publicKey, networkName, GAS_PRICE, SEND_TTL ) // create the deploy params
  const toPublicKey = CLPublicKey.fromHex( to ) // create the to public key
  const session = DeployUtil.ExecutableDeployItem.newTransfer( amount, toPublicKey, null, id ) // create the session
...
module.exports = { // export the functions
  start: async ( url, isTest, userSeed ) => { // start the casper
    URL = url // set the url
  ...
  getPrivateKey: async (seed, index) => ( await getAccount( seed, index, true ) ).privateKey, // get the private key
  getAddress: async (seed, index) => ( await getAccount( seed, index ) ).address, // get the address
  balance: async (seed, index) => { // get the balance
    try {
  ...
  tx: async (seed, index, destination, amount) => { // send a transfer
    try {
      return sendTransfer(new Date().getTime(), 'casper' + ( TESTNET? '-test': '' ), ( await getAccount( seed, index, true ) ), destination, amount) // send the transfer
    } catch (e) {
      return e.message // return the error
    }
  },
  nfts: async address => { // get the nfts
